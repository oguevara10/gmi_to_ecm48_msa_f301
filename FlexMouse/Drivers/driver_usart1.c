...
{
//  usart1SeqMemRX_u32 = (*usart1Control).seqMemRX_u32;   //pam bug without
...
*/
        ...
        Regal_SendDebugMessage(protocolBuf, DataLen2); // !error
#endif
...
{ //process Tx frame in Tx pipe
  ...
  if(indexTx == 0)
  {
    ...
    if(RingBuf_GetUsedNumOfElements((*usart1Control).seqMemTX_u32) >= txHeaderSize)
    {
      ...
      RingBuf_ReadBlock((*usart1Control).seqMemTX_u32, wholeFramebuf, &DataLenTx);             //copy the complete frame into buffer
